...
class SessionInfoManager { 
  ...
  constructor(app) { 
   ...
   app.use(session({secret: 'sss34543hhhhh',saveUninitialized: true,resave: true}));
  }
  PullSessionData(req){
       // console.log('PullSessionData',req.session);
    this.req=req;
     if(req.session.OperationsInfo==undefined)
       this.OperationsInfo={}
    else
      this.OperationsInfo=req.session.OperationsInfo;
  }
  SaveSessionData(){
  ...
  {     if(this.OperationsInfo==undefined)
       this.OperationsInfo={}
  ...
  GetProgress(pOperationKEy){
    ...
    if(this.OperationsInfo[pOperationKEy]==undefined)
      ...
      return this.OperationsInfo[pOperationKEy].Progress;
